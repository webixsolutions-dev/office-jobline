/** All page imagery in one place so assets can be swapped later. */
export const images = {
  aboutHero: '/img1.webp',
  aboutMission: '/img2.webp',
  testimonialJessica: '/img3.webp',
  testimonialMark: '/img4.webp',
  browseHero: '/img5.webp',
  contactHero: '/img6.webp',
  employersHero: '/img7.webp',
  homeHero: '/img8.webp',
  homeHowItWorks: '/img9.webp',
  articleResumeTips: '/img10.webp',
  articleInterviewTips: '/img11.webp',
  articleHiringSupport: '/img12.webp',
  postJobHero: '/img13.webp',
  signInHero: '/img2.webp',
  // Static placeholder map for Visit Our Office — not a live Maps API embed.
  officeMap: '/map-toronto.svg',
}
